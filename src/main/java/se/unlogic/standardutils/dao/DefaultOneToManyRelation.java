...
public class DefaultOneToManyRelation<LocalType,RemoteType> implements OneToManyRelation<LocalType, RemoteType> {
	...
	public DefaultOneToManyRelation(Class<LocalType> beanClass, Class<RemoteType> remoteClass, String remoteFieldName, Field field, AnnotatedDAOFactory daoFactory, DAOManaged daoManaged) {
		...
		boolean remoteFieldNameSet = !StringUtils.isEmpty(remoteFieldName);
		...
		for (Field remoteField : fields) {
			...
			if (remoteField.getType().equals(beanClass) && remoteField.isAnnotationPresent(DAOManaged.class) && remoteField.isAnnotationPresent(ManyToOne.class)) {
				...
				if (remoteFieldNameSet && remoteFieldName.equals(remoteField.getName())) {
					this.remoteField = remoteField;

					break;
					
				} else if (!remoteFieldNameSet) {
					if (this.remoteField != null) {
						throw new RuntimeException("Found multiple corresponding @ManyToOne fields in class " + remoteClass + " for @OneToMany annotated field " + field.getName() + " in " + beanClass);
					}
	...
	public static <LT,RT> OneToManyRelation<LT, RT> getGenericInstance(Class<LT> beanClass, Class<RT> remoteClass, String remoteFieldName, Field field, AnnotatedDAOFactory daoFactory, DAOManaged daoManaged){

		return new DefaultOneToManyRelation<LT,RT>(beanClass, remoteClass, remoteFieldName, field, daoFactory, daoManaged);
	}
}
